...
print((colnames(counts))[treated_samples])
wt_condition <- data.frame(patients = wt_samples, condition = 'wt')
print("wt samples:")
print((colnames(counts))[wt_samples])
...
if (which_results == 'down') {
  ...
  DEres <- res[res$log2FoldChange > 0, ]
} else {  #differentially expressed!
  DEres <- res
}
DEres <- na.omit(DEres)
#return(list(DEres=DEres, conditions = samples$condition))
return(DEres)
}
